...
3. **单飞 + 缓存**：同 key 并发合并为一次在途请求；报价 5s / K 线 5min TTL 缓存减少重复请求。
...
| 问题 | 级别 | 说明 |
|---|---|---|
| `isTransientError` 把**所有** `TypeError` 当网络错误 | 轻 | 会掩盖代码 bug，但重试无副作用 |
| `high: num(33) \|\| num(41)` 当 f[33]=0 时回退 | 轻 | 停牌股边缘；建议改 `??` |
| `todayUtc()` 用 UTC「今天」，北京时间凌晨差一天 | 轻 | 历史查询影响可忽略 |
| `TtlCache` 无 max size / 主动淘汰 | 轻 | 长进程累积，建议加条目上限 |
| `requestTimeoutMs > timeoutMs(30s)` 时工具超时先触发 | 轻 | 文档应注明 `requestTimeoutMs` 须 < `timeoutMs` |
| US 历史靠 `f[2]` 带 `.OQ/.N` 后缀 | 轻 | AAPL/KO 已实测 OK，个别股可能失败 |
| `week/month` 区间 >12/53 年静默返回最新 640 根 | 轻 | 文档已写「单请求覆盖 12/53 年」，属已知限制 |
