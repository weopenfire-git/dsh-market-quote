...
- **内置限流防封**：全局限速 ≤2 QPS（两次请求间隔 ≥500ms）+ 同 key 单飞 + 报价 5s / K 线 5min 缓存，把并发突发摊平为低频串行请求，避免「一次性要太多」触发上游封禁 / IP 封锁；三项均可配置。
...
- **限流防封（三层）**：进程内 TTL 缓存（报价 5s / K 线 5min）去重；同 key 并发合并为一次在途请求（single-flight）；全局限速器保证任意两次请求间隔 ≥500ms（≤2 QPS），并发调用排队到未来时隙、逐个放行——从源头避免「短时间大量请求」触发上游封禁 / IP 封锁。配合 K 线单请求 640 根、总量 2000 根的上限，单次查询的请求数也有硬边界。时钟与睡眠可注入，单测可确定性驱动时间。
...
- 限流是「固定间隔」的主动防护，无自适应退避 / 重试；上游临时故障会直接抛错、不做重试。
