...
- **Safe retry strategy (per-request)**: every attempt carries an `AbortController` timeout (hung requests fail fast) combined with the owning call's cancellation signal; only transient errors (HTTP 5xx, network failure, timeout) are retried — 4xx (incl. the 429 "stop" signal) is never retried; exponential backoff + full jitter (1s→2s→4s), max 3 retries; a paginated query retries only the failed page, never the whole range; each retry still queues through the global limiter, never cutting the line or amplifying QPS.
